...
knitr::opts_knit$set(root.dir = "C:/Users/noahs/PycharmProjects/tagseq-qiime2-snakemake-1/scratch/qiime2/asv")
list.files()
```
...
asv_table = read.table("mld-test-fecal-asv-table.tsv " ,  header = F)
...
```{r}
metadata <- read.table("sample-metadata.tsv", row.names = 1, header = TRUE)
```
...
```
```{r}
print(my_pres, target = "pilot_resp-microbiome.pptx")
```

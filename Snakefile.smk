...
rule drop_blanks:
  ...
  shell:
      ...
      if [ "${{params.var}}" == 'yes' ]; then
        qiime feature-table filter-samples –i-table {input.table} –m-metadata-file {params.metadata} --p-exclude-ids TRUE --p-where {params.blanks} –o-filtered-table {output.cleaned_table} 
      elif [ "${{var}}" == 'no' ]; then
